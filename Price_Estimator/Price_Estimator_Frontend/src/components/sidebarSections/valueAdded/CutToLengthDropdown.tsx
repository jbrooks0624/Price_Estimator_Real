...
const CutToLengthDropdown: React.FC = () => {
  ...
  const skipped = state.cut.skipped;
  const segments = state.cut.segments as Segment[];
  ...
  const setSkipped = (next: boolean) =>
    setState((prev) => ({ ...prev, cut: { ...prev.cut, skipped: next } }));

  const writeSegments = (nextSegments: Segment[]) =>
    setState((prev) => ({
      ...prev,
      cut: { ...prev.cut, segments: nextSegments },
    }));

  const setCost = (val: string) =>
    setState((prev) => ({
      ...prev,
      cut: { ...prev.cut, cost: format(val, 4) },
    }));
  ...
  return (
    <ProcessDropdown title="Cut To Length (Optional)">
      ...
      <div className="space-y-4">
        ...
        {/* Cost input under all segment controls */}
